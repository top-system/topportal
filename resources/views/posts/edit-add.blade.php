...
@section('page_title', __('admin::generic.'.($edit ? 'edit' : 'add')).' '.$dataType->getTranslatedAttribute('display_name_singular'))

@section('page_header')
    <h1 class="page-title">
        <i class="{{ $dataType->icon }}"></i>
        {{ __('admin::generic.'.($edit ? 'edit' : 'add')).' '.$dataType->getTranslatedAttribute('display_name_singular') }}
    </h1>
...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if($edit){{ route('admin.posts.update', $dataTypeContent->id) }}@else{{ route('admin.posts.store') }}@endif" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <input type="text" class="form-control" id="title" name="title" placeholder="{{ __('admin::generic.title') }}" value="{{ $dataTypeContent->title ?? '' }}">
                        </div>
                    </div>
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ __('admin::post.content') }}</h3>
                            <div class="panel-actions">
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ __('admin::post.additional_fields') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action admin-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                            ...
                            @foreach($dataTypeRows as $row)
                                @if(!in_array($row->field, $exclude))
                                    ...
                                    @if (isset($row->details->formfields_custom))
                                        @include('admin::formfields.custom.' . $row->details->formfields_custom)
                                    @else
                                        <div class="form-group @if($row->type == 'hidden') hidden @endif @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            ...
                                            @if($row->type == 'relationship')
                                                @include('admin::formfields.relationship', ['options' => $row->details])
                                            @else
                                                {!! app('admin')->formField($row, $dataType, $dataTypeContent) !!}
                                            @endif
                ...
                <div class="col-md-4">
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                <label for="slug">{{ __('admin::post.slug') }}</label>
                                @include('admin::multilingual.input-hidden', [
                                    '_field_name'  => 'slug',
                            ...
                            <div class="form-group">
                                <label for="status">{{ __('admin::post.status') }}</label>
                                <select class="form-control" name="status">
                                    <option value="PUBLISHED"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'PUBLISHED') selected="selected"@endif>{{ __('admin::post.status_published') }}</option>
                                    <option value="DRAFT"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'DRAFT') selected="selected"@endif>{{ __('admin::post.status_draft') }}</option>
                                    <option value="PENDING"@if(isset($dataTypeContent->status) && $dataTypeContent->status == 'PENDING') selected="selected"@endif>{{ __('admin::post.status_pending') }}</option>
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="featured">{{ __('admin::generic.featured') }}</label>
                    ...
                    <div class="panel panel-bordered panel-info">
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                <label for="meta_description">{{ __('admin::post.meta_description') }}</label>
                                @include('admin::multilingual.input-hidden', [
                            ...
                            <div class="form-group">
                                <label for="meta_keywords">{{ __('admin::post.meta_keywords') }}</label>
                                @include('admin::multilingual.input-hidden', [
                                    '_field_name'  => 'meta_keywords',
                                    '_field_trans' => get_field_translations($dataTypeContent, 'meta_keywords')
                                ])
                            ...
                            <div class="form-group">
                                <label for="seo_title">{{ __('admin::post.seo_title') }}</label>
                                @include('admin::multilingual.input-hidden', [
            ...
            @section('submit-buttons')
                <button type="submit" class="btn btn-primary pull-right">
                     @if($edit){{ __('admin::post.update') }}@else <i class="icon wb-plus-circle"></i> {{ __('admin::post.new') }} @endif
                </button>
            @stop
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="confirm_delete">{{ __('admin::generic.delete_confirm') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('document').ready(function () {
            ...
            $('#confirm_delete').on('click', function(){
                $.post('{{ route('admin.'.$dataType->slug.'.media.remove') }}', params, function (response) {
                    if ( response
                        && response.data
