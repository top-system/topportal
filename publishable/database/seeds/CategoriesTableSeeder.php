...
class CategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                'slug'                  => 'categories',
                'display_name_singular' => __('admin::seeders.data_types.category.singular'),
                'display_name_plural'   => __('admin::seeders.data_types.category.plural'),
                'icon'                  => 'admin-categories',
                'model_name'            => 'TopSystem\\TopPortal\\Models\\Category',
                'controller'            => '',
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'display_name' => __('admin::seeders.data_rows.parent'),
                'required'     => 0,
                'browse'       => 0,
                'read'         => 1,
                'edit'         => 1,
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('admin::seeders.data_rows.order'),
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($categoryDataType, 'seo_title');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($categoryDataType, 'meta_keywords');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text_area',
        ...
        $menuItem = MenuItem::firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('admin::seeders.menu_items.categories'),
            'url'     => '',
